#ifndef OPENCALC_OPENCALC_H
#define OPENCALC_OPENCALC_H
 ...
 */
float height_3(float initial_velocity, float duck_height, float angle, float g_constant);
...
float height_3(float initial_velocity, float duck_height, float angle, float g_constant){
    return ((2*(tanf(angle)* tanf(angle))*(initial_velocity* cosf(angle)-duck_height)*duck_height)/g_constant);
}

#endif //OPENCALC_OPENCALC_H
