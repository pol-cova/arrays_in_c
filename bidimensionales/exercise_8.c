...
//
/*
 * Lea una matriz de tamaño NxM. Calcule la suma de los elementos de una fila seleccionada por el usuario.
 */
...
int main(){
    ...
    printf("\n Seleccione una de las filas disponibles : %d \n", n_vector);
    scanf("%d", &user_column);
    user_column = user_column - 1;


    printf("\n Su fila selecciona fue: %d \n", user_column + 1);
    for (int i = 0; i < m_vector; i++){
        sum_of_column = sum_of_column + user_vector[user_column][i];
    }
